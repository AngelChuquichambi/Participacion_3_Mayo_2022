...
public class VentaDAOimpl extends ConexionDB implements VentaDAO {
    ...
    public List<Venta> getAll() throws Exception {
        ...
        try {
            this.conectar();
            PreparedStatement ps = this.conn.prepareStatement("select v.id as id, c.nombre as nom_cliente, p.nombre as nom_producto, v.fecha as fecha from clientes c join ventas v on ( c.id = v.cliente_id ) join productos p on ( v.producto_id = p.id );");
            //SELECT v.*,p.nombre as producto, c.nombre as cliente FROM ventas v LEFT JOIN productos p ON v.producto_id = p.id LEFT JOIN clientes c ON v.cliente_id = c.id
            ResultSet rs = ps.executeQuery();
            // Iniciar la lista 
            ...
            while (rs.next()) {
                Venta v = new Venta();
                v.setId(rs.getInt("id"));
                v.setCliente_id(rs.getInt("id"));
                v.setProducto_id(rs.getInt("id"));
                v.setFecha(rs.getDate("fecha"));
                /*v.setId(rs.getInt("id"));
                v.setCliente_id(rs.getInt("cliente_id"));
                v.setId(rs.getInt("id"));
